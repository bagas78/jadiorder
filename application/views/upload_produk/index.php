...
<div class="card-pj">
	...
	<br/>
	<div class="card">
		<div class="card-header row" style="background-color: unset; border-bottom: 0">
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#impor").on("submit",function(e){
				...
				$.ajax( {
	                url        : '<?php echo site_url("atmin/api/import"); ?>',
	                type       : 'POST',
	                contentType: false,
	                cache      : false,
	                processData: false,
		...
		function sundul(id){
			$.post("<?=site_url("atmin/produk/sundul")?>",{"id":id},function(msg){
				var data = eval("("+msg+")");
		...
		function hapus(id){
			swal.fire({
				...
				if(val.value == true){
					$.post("<?=site_url("atmin/api/hapusproduk")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
						updateToken(data.token);
						if(data.success == true){
