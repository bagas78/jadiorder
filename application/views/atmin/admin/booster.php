...
<script type="text/javascript">
		...
		$("#sbform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/api/tambahbooster")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function saveSetting(val){
		...
		$.post("<?=site_url('atmin/api/savesetting')?>",{"notif_booster":val,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			if(val == 1){
	...
	function loadSB(page){
		...
		$.post("<?=site_url("atmin/api/booster?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function edit(id){
		$.post("<?=site_url('atmin/api/booster')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapusbooster")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadSB(1);
						...
						<select id="usrid" name="usrid" class="select2" required >
							<?php
								...
								foreach($dbs->result() as $rs){
									echo "<option value='".$rs->usrid."'>".$this->admfunc->clean($rs->nama)." ".$this->admfunc->clean($rs->nohp)."</option>";
								}
							?>
						</select>
					</div>
