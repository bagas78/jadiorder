...
<script type="text/javascript">
		...
		$("#voucherform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/api/tambahvoucher")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadVoucher(page){
		...
		$.post("<?=site_url("atmin/api/voucher?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function edit(id){
		$.post("<?=site_url('atmin/api/voucher')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#voucherid").val(id);
			$("#nama").val(data.nama);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapusvoucher")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadVoucher(1);
