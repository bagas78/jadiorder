...
<script type="text/javascript">
		...
		$("#sbforms").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.ajax({
						url        : '<?php echo site_url("atmin/api/tambahtestimoni"); ?>',
						type       : 'POST',
						contentType: false,
						cache      : false,
						processData: false,
	...
	function loadTesti(page){
		...
		$.post("<?=site_url("atmin/api/testimoni?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function edit(id){
		$.post("<?=site_url('atmin/api/testimoni')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
			$("#nama").val(data.nama);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapustestimoni")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
