...
<script type="text/javascript">
	$(function(){
		...
		$("#sbforms").on("submit",function(e){
			...
			$.post("<?=site_url("atmin/api/tambahtopup")?>",datar,function(data){
				$("#sbforms").show();
				$(".progress").hide();
				var res = eval("("+data+")");
	...
	function loadTopup(page){
		...
		$.post("<?=site_url("atmin/api/topup?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function verifikasi(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/updatetopup")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
	...
	function batal(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/bataltopup")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
