...
<script type="text/javascript">
		...
		$("#wasapform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/api/tambahwasap")?>",datar,function(msg){
						var data = eval("("+msg+")");
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
					$.post("<?=site_url("atmin/api/tambahrekening")?>",datar,function(msg){
						var data = eval("("+msg+")");
						updateToken(data.token);
						if(data.success == true){
							loadSettingBank(1);
				...
				$("#userform").on("submit",function(e){
					...
					swal.fire({
						...
						if(vals.value){
							...
							datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
							$.post("'.site_url("atmin/api/tambahuser").'",datar,function(msg){
								var data = eval("("+msg+")");
								updateToken(data.token);
								if(data.success == true){
									loadUser(1);
	...
	function loadSettingBank(page){
		...
		$("#load").load("<?=site_url("atmin/api/rekening?load=setting&page=")?>"+page);
	}
	function loadWasap(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url("atmin/api/wasap?load=wasap&page=")?>"+page);
	}
	function loadSettingUmum(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('atmin/api/setting')?>");
	}
	...
	function loadSettingServer(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('atmin/api/settingserver')?>");
	}
	function loadSettingPayment(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('atmin/api/settingpayment')?>");
	}
	function loadUser(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url('atmin/api/usermanajer')?>?page="+page,{[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
	...
	function editWasap(id){
		$.post("<?=site_url('atmin/api/wasap')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#wasid").val(id);
	...
	function edit(id){
		$.post("<?=site_url('atmin/api/rekening')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#rekid").val(id);
	...
	<?php
		if($this->func->demo() != true){
			...
			function editUser(id){
				$.post("'.site_url('atmin/api/usermanajer').'",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
					var data = eval("("+ev+")");
					updateToken(data.token);
					$("#usrid").val(id);
			...
			function hapusUser(id){
				swal.fire({
					...
					if(vals.value){
						$.post("'.site_url("atmin/api/hapususer").'",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
							var data = eval("("+msg+")");
							updateToken(data.token);
							if(data.success == true){
								loadUser(1);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapusrekening")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadSettingBank(1);
	...
	function hapusWasap(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapuswasap")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadUser(1);
