	...
	<div class="card">
		<div class="card-header align-items-center">
			<div class="card-title">
				<a class="float-right btn btn-primary" href="<?=site_url("atmin/manage/editblog")?>"><i class="fas fa-plus-circle"></i> Postingan Baru</a>
...
<script type="text/javascript">
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/api/updateblog")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadBlog(page){
		...
		$.post("<?=site_url("atmin/manage/blog?load=hal&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function hapus(pro){
		swal.fire({
			...
			if(result.value){
				$.post("<?php echo site_url('atmin/api/hapusblog'); ?>",{"id":pro,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
