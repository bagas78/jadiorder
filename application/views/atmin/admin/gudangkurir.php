		...
		$(".kurir").each(function(){
			$(this).click(function(){
				...
				if($(this).data("aktif") == "no"){
					$.post("<?php echo site_url("atmin/gudang/aktifkankurir/".$id); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
					...
					if(hitungJumlah() > 1){
						$.post("<?php echo site_url("atmin/gudang/nonaktifkankurir/".$id); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
							var data = eval("("+msg+")");
							updateToken(data.token);
							if(data.success == true){
