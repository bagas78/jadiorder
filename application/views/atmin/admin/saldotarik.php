...
<script type="text/javascript">
	...
	function loadTopup(page){
		...
		$.post("<?=site_url("atmin/api/tarik?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function verifikasi(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/updatetarik")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
	...
	function batal(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/bataltarik")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
