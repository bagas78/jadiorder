...
<script type="text/javascript">
	$(function(){
		...
		$("#kurirform").on("submit",function(e){
            ...
            $.post("<?=site_url("atmin/api/kurirsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
		...
		$("#paketform").on("submit",function(e){
            ...
            var datar = $(this).serialize();
            datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
            $.post("<?=site_url("atmin/api/paketsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                if(data.success == true){
                    loadKurir(1);
		...
		$("#ongkirform").on("submit",function(e){
            ...
            var datar = $(this).serialize();
            datar = datar + "&" + $("#names").val() + "=" + $("#tokens").val();
            $.post("<?=site_url("atmin/api/ongkirsave")?>",datar,function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                if(data.success == true){
                    loadOngkir(1);
        ...
        $("#prov").change(function(){
            $.post("<?=site_url("atmin/api/getkab")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kab").html(data.html);
            });
        ...
        $("#kab").change(function(){
            $.post("<?=site_url("atmin/api/getkec")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kec").html(data.html);
            });
        ...
        $("#ongkirkurir").change(function(){
            $.post("<?=site_url("atmin/api/getpaketdrop")?>",{"id":$(this).val(),[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
    ...
    function selectKec(prov,kab,kec){
        $.post("<?=site_url("atmin/api/getkab/")?>"+kab,{"id":prov,[$("#names").val()]:$("#tokens").val()},function(msg){
            var data = eval("("+msg+")");
            updateToken(data.token);
            $("#kab").html(data.html);
            
			/*
            $.post("<?=site_url("atmin/api/getkec/")?>"+kec,{"id":kab,[$("#names").val()]:$("#tokens").val()},function(msg){
                var data = eval("("+msg+")");
                updateToken(data.token);
                $("#kec").html(data.html);
            });
    ...
    function selectPaket(kurir,paket){
        $.post("<?=site_url("atmin/api/getpaketdrop/")?>"+paket,{"id":kurir,[$("#names").val()]:$("#tokens").val()},function(msg){
            var data = eval("("+msg+")");
            updateToken(data.token);
            $("#ongkirpaket").html(data.html);
        });
	...
	function loadKurir(page){
		...
		$("#load").load("<?=site_url("atmin/api/kurir?page=")?>"+page);
	}
	function loadOngkir(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url("atmin/api/ongkir?page=")?>"+page);
	}
	function loadSettingKurir(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('atmin/api/settingkurir')?>");
	}
	function editKurir(id){
		$.post("<?=site_url('atmin/api/getkurir')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$('#kurirform')[0].reset();
            $('#kuririd').val(data.id);
	...
	function editPaket(id){
		$.post("<?=site_url('atmin/api/getpaket')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            $('#paketform')[0].reset();
            $('#paketid').val(data.id);
	...
	function editOngkir(id){
		$.post("<?=site_url('atmin/api/getongkir')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            $('#ongkirform')[0].reset();
	...
	function hapusKurir(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapuskurir")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadKurir(1);
	...
	function hapusPaket(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapuspaket")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadKurir(1);
	...
	function hapusOngkir(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapusongkir")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadOngkir(1);
