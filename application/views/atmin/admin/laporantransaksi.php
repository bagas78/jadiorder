...
<script type="text/javascript">
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/api/update")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadRiwayat(){
		...
		$.post("<?=site_url("atmin/manage/laporantransaksi?load=hal")?>",{"gudang":$("#gudang").val(),"status":$("#status").val(),"jenis":$("#jenis").val(),"tglmulai":$("#tglmulai").val(),"tglselesai":$("#tglselesai").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
