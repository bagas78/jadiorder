<div style="">
	<a class="float-right btn btn-primary" href="<?=site_url("atmin/manage/sliderform")?>"><i class="fas fa-plus-circle"></i> Tambah Promo</a>
	...
	<div class="card">
		...
		<?php
			...
			foreach($pro->result() as $r){
				...
				$tipe = ($r->jenis == 4) ? "<b class='text-warning'>POPUP BANNER</b>" : $tipe;
		...
		?>
			<tr>
				...
				<td>
					<a href="<?php echo site_url("atmin/manage/sliderform/".$r->id); ?>" class="btn btn-primary"><i class="fas fa-pencil-alt"></i> Edit</a>
...
<script type="text/javascript">
	...
	function hapusPromo(pro){
		swal.fire({
			...
			if(result.value){
				$.post("<?php echo site_url('atmin/manage/hapusslider'); ?>",{"pro":pro,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
	...
	function refreshTabel(page){
		window.location.href = "<?php echo site_url('atmin/manage/slider/?page='); ?>"+page;
	}
</script>