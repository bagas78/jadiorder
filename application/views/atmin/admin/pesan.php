...
<script type="text/javascript">
	...
	function loadSemua(page){
		...
		$.post("<?=site_url("atmin/manage/pesan?load=semua&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function loadPesan(nul){
		var id = $("#temp").val();
		$("#pesan").load("<?=site_url("atmin/api/pesanmasuk")?>/"+id,function(){
			if(nul == 0){
...
<script type="text/javascript">
	$(function(){
		$("#kirimpesan").on("submit",function(e){
			...
			$.post("<?=site_url("atmin/api/kirimpesan")?>",datar,function(s){
				var data = eval("("+s+")");
				updateToken(data.token);
