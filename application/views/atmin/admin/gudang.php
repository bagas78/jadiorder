...
<script type="text/javascript">
		...
		$("#sbforms").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/gudang/tambah")?>",datar,function(msg){
                        var res = eval("("+msg+")");
	...
	function loadGudang(page){
		...
		$.post("<?=site_url("atmin/gudang/data?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function edit(id){
		$.post("<?=site_url('atmin/gudang/data')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
			$("#nama").val(data.nama);
	...
	function editKurir(id){
		$("#loadmodal").load("<?=site_url("atmin/gudang/setkurir")?>/"+id,function(){
			$("#modalkurir").modal();
		});
	}
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/gudang/hapus")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
