...
<script type="text/javascript">
	$(function(){
		...
		$("#cetakInvoice").on("submit",function(e){
			e.preventDefault();
			window.open("<?=site_url("atmin/api/cetakInvoice")?>"+"?id="+$("#inv").val());
		});
		$("#cetakInvoiceThermal").on("submit",function(e){
			e.preventDefault();
			window.open("<?=site_url("atmin/api/cetakInvoice")?>"+"?type=thermal&id="+$("#inv").val());
		});
		
		$("#cari").change(function(){
			var load = $(".tabs-item.active").data("item");
			$.post("<?=site_url("atmin/api/pesanan")?>?page=1&load="+load,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
				var data = eval("("+msg+")");
	...
	function loadBayar(page){
		...
		$.post("<?=site_url("atmin/api/pesanan?load=bayar&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadDikemas(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/api/pesanan?load=dikemas&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadDikirim(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/api/pesanan?load=dikirim&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadSelesai(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/api/pesanan?load=selesai&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadBatal(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/api/pesanan?load=batal&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function detail(id){
		...
		$("#detailoader").show();
		$("#detaiload").hide();
		$("#modaldetail .modal-body").load("<?=site_url('atmin/api/detailpesanan')?>?theid="+id,function(){
			$("#detailoader").hide();
			$("#detaiload").show();
			$("#inv").val(id);
		});
	...
	function cetak(inv){
		window.open("<?=site_url("atmin/api/cetakInvoice")?>"+"?id="+inv);
	}
	function cetakThermal(inv){
		window.open("<?=site_url("atmin/api/cetakInvoice")?>"+"?type=thermal&id="+inv);
	}
	function cetakLabel(id){
		$.post("<?=site_url("atmin/api/cetakLabel")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function lacakPaket(orderid){
		...
		$("#lacakloader").show();
		$("#lacakload").hide();
		$("#modalacak #lacakload").load("<?=site_url('atmin/api/lacakiriman')?>?orderid="+orderid,function(){
			$("#lacakloader").hide();
			$("#lacakload").show();
		});
	}
	function selesai(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/terimapesanan")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
	...
	function batalkan(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/batalkanpesanan/bymin")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadBatal(1);
