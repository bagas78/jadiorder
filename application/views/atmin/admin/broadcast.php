...
<script type="text/javascript">
	$(function(){
		...
		$("#forms").on("submit",function(e){
			...
			$.ajax( {
                url        : '<?=site_url("atmin/api/tambahbroadcast")?>',
                type       : 'POST',
                contentType: false,
                cache      : false,
                processData: false,
	...
	function loadB(page){
		...
		$.post("<?=site_url("atmin/api/broadcast?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/hapusbroadcast")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
	...
	function resend(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/api/updatebroadcast")?>",{"id":id,"status":0,[$("#names").val()]:$("#tokens").val()},function(data){
					var res = eval("("+data+")");
					updateToken(res.token);
					if(res.success == true){
