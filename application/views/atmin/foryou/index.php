...
<h4 class="page-title m-b-20">For You Produk</h4>

<div class="m-b-60">
	<div class="card">
		<div class="card-header row align-items-center">
			<div class="card-title col-md-8 m-b-10">
				<a href="javascript:tambah()" class="btn btn-primary"><i class="fas fa-plus-circle"></i> Tambah</a>
			</div>
			<div class="col-md-4">
...
<script type="text/javascript">
	$(function(){
		...
		$("#sbform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("atmin/foryou/tambah")?>",datar,function(msg){
						var data = eval("("+msg+")");
        ...
        $("#produk").change(function(){
            var id = $(this).val();
            $.post("<?=site_url()?>/atmin/foryou/getproduk/"+id,{[$("#names").val()]:$("#tokens").val()},function(msg){
				var data = eval("("+msg+")");
				updateToken(data.token);
	...
	function load(page){
		...
		$.post("<?=site_url("atmin/foryou/data?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function edit(id){
		...
		$.post("<?=site_url('atmin/foryou/data')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            //$("#loadproduk").hide();
			$("#id").val(id);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/foryou/hapus")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						load(1);
