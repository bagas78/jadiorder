...
<script type="text/javascript">
		...
		$("#sbform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.ajax({
						url        : '<?php echo site_url("atmin/brand/tambah"); ?>',
						type       : 'POST',
						contentType: false,
						cache      : false,
						processData: false,
	...
	function load(page){
		...
		$.post("<?=site_url("atmin/brand/data?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function edit(id){
		...
		$.post("<?=site_url('atmin/brand/data')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
            //$("#loadproduk").hide();
			$("#id").val(id);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("atmin/brand/hapus")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
