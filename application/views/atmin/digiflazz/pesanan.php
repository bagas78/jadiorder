...
<script type="text/javascript">
	$(function(){
		...
		$("#cari").change(function(){
			...
			$.post("<?=site_url("atmin/ppob/pesananload")?>?page=1&load="+load,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
				var data = eval("("+msg+")");
	...
	function loadProses(page){
		...
		$.post("<?=site_url("atmin/ppob/pesananload?load=proses&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadSelesai(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/ppob/pesananload?load=selesai&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function loadBatal(page){
		...
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("atmin/ppob/pesananload?load=batal&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function cekPesanan(id){
		...
		$("#modalstatus").modal();
		$("#modalstatus .modal-body").load("<?=site_url("atmin/ppob/status")?>/"+id);
	}
</script>
