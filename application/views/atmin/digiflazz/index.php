	...
	<div class="card">
		<div class="card-header">
			<div class="tabs p-lr-15 m-b-12">
				<a href="javascript:void(0)" onclick="loadProduk(1);$('.tkategori').hide();" class="tabs-item active">
					<i class="fas fa-box"></i> &nbsp;Produk Prabayar
				</a>
				<a href="javascript:void(0)" onclick="loadProdukPasca(1);$('.tkategori').hide();" class="tabs-item">
					<i class="fas fa-clock"></i> &nbsp;Produk Pascabayar
				</a>
				<a href="javascript:void(0)" onclick="loadKategori(1);$('.tkategori').show();" class="tabs-item">
					<i class="fas fa-list"></i> &nbsp;Kategori Produk
				</a>
				<a href="javascript:void(0)" onclick="loadPengaturan();$('.tkategori').hide();" class="tabs-item">
...
<script type="text/javascript">
		...
		$("#proform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.ajax({
						url        : '<?php echo site_url("atmin/ppob/saveproduk"); ?>',
						type       : 'POST',
						contentType: false,
						cache      : false,
						processData: false,
	...
	function loadProduk(page){
		...
		$("#load").load("<?=site_url("atmin/ppob/produk?page=")?>"+page);
	}
	function loadProdukPasca(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url("atmin/ppob/produkpasca?page=")?>"+page);
	}
	function loadKategori(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url("atmin/ppob/kategori?page=")?>"+page);
	}
	function loadPengaturan(){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$("#load").load("<?=site_url('atmin/ppob/setting')?>");
	}
	function editProduk(id){
		$.post("<?=site_url('atmin/ppob/getproduk')?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
